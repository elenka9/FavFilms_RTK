...
import { removeMovie, clearMovies, findMovieByTitle } from "../../movie/favoriteMoviesSlice";
...
const FavoriteMoviesList = () => {
  ...
  // const filtredMovies = movies.filter((movie => movie.title.toLowerCase().includes(findByTitle.toLowerCase())))
  ...
  return (
    <div >
      ...
      />
      <button onClick = {()=>dispatch(findMovieByTitle(findByTitle))}>Find</button>
      ...
      <ul className ='zebra'>
        {movies.map((movie) => (
          <li key={movie.id}>
            {movie.title}
            <button className ='delete' onClick={() => dispatch(removeMovie(movie.id))}>
              Delete
            </button>
          </li>
